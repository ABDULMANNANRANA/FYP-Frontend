...
import { putJson } from '../../services/locationApi';
...
/**
 * Edit a LOCATION BASED task.
 *
 * Same three-radio family as the Add screens. Carries the task through from
 * whichever screen the user arrived from, so nothing typed is lost when
...
const EditTaskLocationBased = ({ navigation, route }) => {
  ...
  // Prefilled from the task's saved place, editable here.
  const [geofence, setGeofence] = useState({
    latitude: task?.latitude ?? task?.Latitude ?? null,
    longitude: task?.longitude ?? task?.Longitude ?? null,
    accuracy: null,
    geofenceRadiusMeters:
      task?.geofenceRadiusMeters ??
      task?.GeofenceRadiusMeters ??
      DEFAULT_GEOFENCE_RADIUS_METERS,
    geofenceEnabled: task?.geofenceEnabled ?? task?.GeofenceEnabled ?? true,
  });

  const hasPlace = geofence.latitude !== null && geofence.longitude !== null;
  ...
  const inputBg = theme.inputBg || (isDark ? '#0F172A' : '#F8FAFC');

  // ------------------------------------------------- type switching

  // Whatever is on screen now travels with the user, so switching type
  // never loses edits.
  const carriedTask = {
    ...task,
    title,
  ...
  const switchToTimeBased = () => {
    navigation.navigate('EditTaskTimeBased', { task: carriedTask });
  };

  const switchToNonTimeBased = () => {
    navigation.navigate('EditTaskNonTimeBased', { task: carriedTask });
  };
  ...
  const saveTask = async () => {
    if (!task?.id) {
      Alert.alert('Error', 'This task could not be loaded.');
      return;
    }
    ...
    try {
      ...
      const data = await putJson(`/Managment/${task.id}`, {
        title: title.trim(),
      ...
      Alert.alert('Updated', data.message || 'Task updated successfully.', [
      ...
      >
        ...
        <View style={[styles.card, { backgroundColor: cardBg, borderColor: borderClr }]}>
          ...
          <View style={styles.radioContainer}>
            {/* (1) TIME BASED */}
            <TouchableOpacity
              style={[styles.radioItem, { borderColor: borderClr }]}
...
export default EditTaskLocationBased;
